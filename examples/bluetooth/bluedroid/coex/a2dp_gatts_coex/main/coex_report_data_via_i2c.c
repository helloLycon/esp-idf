...
SemaphoreHandle_t msg_mutex = NULL;
...
void i2c_comm_app_main()
{
    ...
    bt_msg.frame = (uint8_t *)malloc(DATA_LENGTH);
    unsigned char *send_frame = (uint8_t *)malloc(DATA_LENGTH);
    uint8_t *data_rd = (uint8_t *)malloc(256);

    if(NULL == bt_msg.frame || NULL == send_frame ||  NULL == data_rd) {
        ESP_LOGE(TAG, "malloc() failed");
        return;
    }

    strcpy((void *)bt_msg.frame, hdr);
    const unsigned short init_total_len = strlen(hdr) + 2;
    bt_msg.total_len = init_total_len;
    bt_msg_ready = true;
    for(;;) {
        ...
        if(ret >= 0) {
            if(ret) {
                xSemaphoreTake(msg_mutex, portMAX_DELAY);
                unsigned short send_len = bt_msg.total_len;
                *(unsigned short *)(bt_msg.frame + strlen(hdr)) = htons(send_len - init_total_len);
                memcpy(send_frame, bt_msg.frame, send_len);
                bt_msg.total_len = init_total_len;
                xSemaphoreGive(msg_mutex);
                set_checksum(send_frame + init_total_len, send_len - init_total_len);
                send_len += 2;
                printf("send_len = %d, clear bt_msg.total_len\n", send_len);
                disp_buf(send_frame, send_len);
                i2c_slave_write_buffer(I2C_SLAVE_NUM, send_frame, send_len, 5000 / portTICK_RATE_MS);
            } else {
                printf("no data\n");
            }
        }
